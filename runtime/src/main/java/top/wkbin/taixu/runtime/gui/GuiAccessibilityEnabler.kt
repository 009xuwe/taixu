...
class GuiAccessibilityEnabler @Inject constructor(
    ...
    suspend fun ensureEnabled(): Boolean {
        ...
        val component = ComponentName(context, TaiXuGuiAccessibilityService::class.java).flattenToString()
        ...
        val services = current.split(':')
            ...
            .toMutableList()
        if (services.none { it.equals(component, ignoreCase = true) || it.endsWith("/${TaiXuGuiAccessibilityService::class.java.name}") }) {
            services += component
        }
