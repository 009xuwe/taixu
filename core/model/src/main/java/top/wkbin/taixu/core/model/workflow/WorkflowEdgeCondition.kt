...
object WorkflowEdgeCondition {
    ...
    fun validationError(edge: WorkflowEdge): String? {
        ...
        EXIT_CODE.matchEntire(expression)?.let { return null }
    ...
    fun matches(edge: WorkflowEdge, output: NodeExecutionOutput): Boolean {
        ...
        EXIT_CODE.matchEntire(expression)?.let { return output.exitCode == it.groupValues[1].toInt() }
    ...
    private val EXIT_CODE = Regex("^exitCode\\s*==\\s*(-?\\d+)$", RegexOption.IGNORE_CASE)
